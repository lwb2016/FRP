...
namespace UnityEngine.Rendering.Universal
{
    ...
    sealed internal class FURPTaaPersistentData
    {
    ...
    static class FRPTemporalAA
    {
        ...
        internal static void ExecutePass(CommandBuffer cmd, Material taaMaterial, ref CameraData cameraData, ref FRPCamera frpCamera, RTHandle source, RTHandle destination, RenderTexture motionVectors)
        {
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.TemporalAA)))
            {
...
#endif
                bool isNewFrame = frpCamera.taaPersistentData.GetLastAccumFrameIndex(multipassId) != Time.frameCount;

                RTHandle taaHistoryAccumulationTex = frpCamera.taaPersistentData.accumulationTexture(multipassId);
                ...
                ref var taa = ref frpCamera.taaSettings;
                ...
                if (isNewFrame)
                {
                    ...
                    frpCamera.taaPersistentData.SetLastAccumFrameIndex(multipassId, Time.frameCount);
                }
            }
        }
    }
