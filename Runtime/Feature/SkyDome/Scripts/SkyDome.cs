...
namespace UnityEngine.Rendering.FRP
{
    ...
    {
        [Range(0.001f, 1)] public float floorHeight = 0.05f;
        private Mesh mesh;
        [Range(8, 100)] public int h = 12;
        [Range(8, 100)] public int v = 24;
        ...
        public void Generate()
        {
            ...
            for (int i = 0; i <= h; i++)
            {
                ...
                {
                    int index = (i) * (v + 1) + j;
                    Vector3 pos = Quaternion.Euler(180 * i / (float)h, 360 * j / (float)v + 180 + rotation, 0) *
                                  Vector3.up;

                    uv[index] = new Vector4(pos.x, pos.y, pos.z,
                        Mathf.Clamp01((-floorHeight - pos.y) / (1 + floorHeight)));
